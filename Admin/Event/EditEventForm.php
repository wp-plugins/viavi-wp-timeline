...
wp_register_style( 'WordPressTimelineJqueryUiCss', plugins_url().'/viavi-wordpress-timeline/Admin/Css/smoothness/jquery-ui-1.10.3.custom.min.css',array(),'','screen' );
wp_enqueue_style( 'WordPressTimelineJqueryUiCss' );

?>
<div class="viavi-content">
<h2><?php echo _e('Edit Event','viavi-wordpress-timeline'); ?></h2>


    <form id="form_gonder" action="<?php echo admin_url().'admin.php?page=viavi-wordpress-timeline/index.php'; ?>&isvav=EditEventPost" method="post">
    
    <table>
    	<tr>
        	<td>
            <h3><?php echo _e('Event Title','viavi-wordpress-timeline'); ?></h3>
            <input type="text" name="event_title" size="40" value="<?php echo $event['title']; ?>"/>            
            </td>
        	<td>
            <h3><?php echo _e('Timeline Name','viavi-wordpress-timeline'); ?></h3>
            <select name="group_id">
                <option><?php echo _e('Select Timeline...','viavi-wordpress-timeline'); ?></option>
                <?php
	...
	<table>
    	<tr>
        	<td>
            <h3><?php echo _e('Event Time (If Anno Domini)','viavi-wordpress-timeline'); ?></h3>
            </td>
            <td>
            <h3><?php echo _e('Event Time (If Before Christ)','viavi-wordpress-timeline'); ?></h3>
            </td>
        </tr>
        <tr>
        	<td>
            <!--<input type="text" id="MyDate" name="event_date" size="40" value="<?php echo $event_date_val; ?>"/> <?php echo _e('[ ! ] e.g.: 2010-01-30 [Year-Month-Day]','viavi-wordpress-timeline'); ?>-->
                    
            <input type="text" class="WordPressDate" name="event_date" size="1" />
                        
            <?php echo _e('Year','viavi-wordpress-timeline'); ?>
            <input type="text" class="WordPressDateYear" name="event_date_year" size="4" value="<?php echo $event_date_val[0]; ?>"/>
                        
            <?php echo _e('Month','viavi-wordpress-timeline'); ?>
            <input type="text" class="WordPressDateMonth" name="event_date_month" size="2" value="<?php echo $event_date_val[1]; ?>"/>
                        
            <?php echo _e('Day','viavi-wordpress-timeline'); ?>
			...
			<p><small><?php echo _e('If you want you can also add time. [ ! ] e.g.: 14:30:45 [Hour-Minute-Second]','viavi-wordpress-timeline'); ?></small></p>
            </td>
            <td>
            <input type="text" name="event_bc" size="40" id="event_bc_input" value="<?php echo ltrim($event_bc_val,'-'); ?>"/>
			<p><small><?php echo _e('[ ! ] e.g.: 2000','viavi-wordpress-timeline'); ?></small></p>
            </td>
        </tr>
    </table>
    ...
    <br />
        ...
        <input type="submit" value="<?php echo _e('Update Event','viavi-wordpress-timeline'); ?>" class="button" id="gonder_button"/>
